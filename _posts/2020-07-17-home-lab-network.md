---
layout: post
title: 'Home Lab: The Network'
date: 2020-07-17 13:01:31.000000000 -05:00
type: post
parent_id: '0'
published: true
password: ''
...
- Management Network
  - Best Practice: [Don't put this on VLAN 1](https://www.timigate.com/2017/12/native-vlan-why-you-should-not-use-vlan.html)!
...
- [HP 2915](https://support.hpe.com/hpesc/public/docDisplay?docId=emr_na-c04868827) (J9562A): This is basically the "core" switch - this is where the other switches, VM hosts, and router are connected.
- [Aruba 2930F](https://www.arubanetworks.com/products/networking/switches/2930f-series/)-8G (JL258A): I have two of these - these typically feed the access points. They are managed, like the APs, with [Aruba Central](https://www.arubanetworks.com/products/networking/management/central/).
- [Ruckus ICX7150-C12P](https://www.commscope.com/globalassets/digizuite/294043-431-ds-icx-7150.pdf?utm_source=ruckus&utm_medium=redirect): This one feeds all the stuff in the house. Managing these is a pain in the butt, but it's a decent switch otherwise.
- [Foundry FastIron SX800](http://www.dataswitchworks.com/FastIron-SX-800.asp): This is the great-great-great grandfather of the ICX switch. Its sole purpose right now is as ballast (it weighs around 100 lb)

Next phase of building out the lab network will be to move everything to 10-Gigabit Ethernet.

### Wiring

Just like in any environment, neat and tidy wiring makes management a lot easier. I have a color code I try to stick to for patch cables. I'm a big fan of the [Monoprice SlimRun patch cables](https://amzn.to/2COFkYI): They're inexpensive, of good quality for the money, and they're a lot easier to manage. For your wiring in the house and the lab, It's always good to follow [BICSI best practices](https://www.bicsi.org/standards/bicsi-standards/standardization/installation), if for no other reason than as an engineer, you should at least be familiar with them.

Velcro is your friend. I'm a big fan of Rip-Tie's [RipWrap](https://amzn.to/32uSkOk) product. It's slim, plays nice with the slim cables, and isn't overly expensive.

When it comes to bulk cable, I am a big fan of [Commscope Uniprise](https://www.commscope.com/product-type/networking-systems/structured-cabling/uniprise/copper-structured-cabling/twisted-pair-cables/), although [Superior Essex](https://superioressexcommunications.com/wp-content/uploads/2020/01/SPSX_CommLineCard_English.pdf) comes in a close second, especially since it's made right here in Kansas. Don't waste your time with any bulk cable from Amazon or Monoprice - the shipping alone for a box of Cat 6 prices them out of competing with your local supply house (I've been a fan of [Accu-Tech](https://www.accu-tech.com/) for nearly 2 decades, HMU if you need a contact there)

For termination, I generally won't use anything but [Panduit's Mini-Com](https://www.panduit.com/en/products/copper-systems/connectors/jack-modules.html) stuff (major exception is anything going into a structured wiring enclosure, since Panduit doesn't make any brackets for those. If you have some metalworking skills, you could probably adapt something, as most of the Mini-Com line is designed around the standard 1.38” x 2.71” knockout hole found on almost any modular office furniture.) The Panduit terminations are a little more expensive, but it's easy to use and you never have to worry about it failing on you. I own a set or two of modular plug (Don't call them "RJ45") crimpers and probably have some plugs, but it's been years since I've actually used them - Making your own patch cables is a complete waste of time, and any installed cable should always be terminated to a jack.

### Monitoring

This starts to veer into servers (which will be covered in a future post), but you probably want to monitor your network. If for no other reason than the fact that *[dashboards look cool](https://grafana.com/blog/2019/05/16/worth-a-look-public-grafana-dashboards/)*. [SNMP](https://en.wikipedia.org/wiki/Simple_Network_Management_Protocol) has been the standard way of monitoring server infrastructure for decades. While there are numerous tools out there such as [Cacti](https://www.cacti.net/) and [LibreNMS](https://www.librenms.org/), nothing does visualizations quite as impressively as Grafana (which isn't really an NMS in and of itself, but merely a rendering and visualization engine that can pull all kinds of data from all kinds of places. As SNMP starts giving way to REST APIs and webhooks, This is the future.)

![]({{site.baseurl}}/assets/2020/07/Annotation-2020-07-17-132427-1024x522.png)

I currently use [InfluxDB](https://www.influxdata.com/products/influxdb-overview/) as my time-series database ([RRDTool](https://oss.oetiker.ch/rrdtool/) is so last century), with [Grafana](https://grafana.com/) visualizing the data, and an assortment of things collecting data, including [Telegraf](https://www.influxdata.com/time-series-platform/telegraf/) for collecting SNMP. I'm gradually adding some API calls for other stuff like VMWare and Aruba.
...
*Featured Image: Reddit User **u/HotSchmoe** in [r/homelab](https://www.reddit.com/r/homelab/comments/ewk3pb/heard_you_like_gore/)*
