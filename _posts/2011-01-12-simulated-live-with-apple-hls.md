---
layout: post
title: Simulated Live with Wowza and Apple HTTP Live Streaming
date: 2011-01-12 09:35:41.000000000 -06:00
type: post
parent_id: '0'
published: true
password: ''
...
- roku
meta:
  dsq_thread_id: '218113145'
  _edit_last: '1'
  s2mail: 'yes'
  _wp_old_slug: ''
  _googl_shortlink: http://goo.gl/zRWDE
author:
  login: ianbeyer
  ...
  first_name: Ian
  last_name: Beyer
permalink: "/2011/01/12/simulated-live-with-apple-hls/"
---

Last summer, we switched our primary Flash streaming over to [316 Networks](http://solutions.316networks.com "316 Networks ") partly because of the simulated live capability they offered, and partly for the Media Suite backend. We continued to use [Wowza](http://wowzamedia.com "Wowza Media Server") on [EC2](http://aws.amazon.com "Amazon EC2 Cloud Computing") for our mobile users, since the solution works very well. Unfortunately, simulated doesn't work for our mobile users, who are limited to live.
...
Wowza doesn't natively support pseudostreaming (although the [Stream Class](http://www.wowzamedia.com/forums/content.php?162-Server-side-publishing-using-the-Stream-class "Wowza Stream Class ") API does have some scheduling capability -- see [this post](http://blog.ianbeyer.com/2011/01/17/wowza-stream-class/ "Using the Wowza Stream Class")), so we needed some way of broadcasting recordings on a schedule. I could use the VT5 machine and the [Scheduler](http://www.youngmonkey.ca/hands/restaurant/plugins/Scheduler/index.html "VT5/Tricaster scheduler") to replay the recordings on a schedule, but the big downside is that it consumes local bandwidth, which is in short supply.

What I needed was a way of streaming the archive files (high and low bandwidth) that were created by the [Kulabyte](http://www.kulabyte.com/live-video-streaming-software-and-server-products/live-video-streaming-software/live-video-streaming-software-xstream-live-2.html "KulaByte xStream Live encoder") encoder during the live event. Ideally, since I already upload these to 316 for rebroadcast, I'd like to not have to upload it to two locations.

Enter the lovely open-source encoder, [ffmpeg](http://ffmpeg.org/ "ffmpeg"). My concern about using ffmpeg was that re-encoding an already encoded file had the potential of introducing compression artifacts and adding CPU load. I was very happy to find an obscure command-line setting that tells it to copy the input file's audio and video codec directly. The only thing ffmpeg would be doing would be extracting the audio and video streams from the MPEG-4 container and stuffing them into a Flash container without molesting the actual streams at all. Added bonus: ffmpeg can also not only output to RTMP, but can take RTMP as an input as well.
...
Attempt #3: I Downloaded the [ffmpeg 0.61 source](http://ffmpeg.org/releases/ffmpeg-0.6.1.tar.gz "ffmpeg 0.6.1") to my Wowza server on EC2 and crossed my fingers that the build would go off without a hitch. Success! I then transplanted the command line I used on my test box, tweaked the destination server to the local Wowza install, and fired it off. Success again!
